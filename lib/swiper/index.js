    ...
    */
    Swiper.prototype.init = function (mapApi) {
        ...
        myBundlePromise.then(function (myBundle) {
            var layerNb = 0;
            _this.mapApi.layersObj.layerAdded.subscribe(function (layer) {
                // check if loaded layer is inside the config
                _this.config.layers.find(function (layer) { if (layer.id === layer.id) {
                    layerNb++;
                } });
                // if all layers require by the plugin are loaded, start it
                if (layerNb === _this.config.layers.length) {
